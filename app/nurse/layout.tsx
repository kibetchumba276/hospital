...
export default function NurseLayout({ children }: { children: React.ReactNode }) {
  ...
  async function checkUser() {
    ...
    try {
      ...
      if (userData?.role !== 'super_admin' && userData?.role !== 'nurse') {
        router.replace('/login')
        return
      }
      ...
      <header className="bg-white border-b sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-2xl font-bold text-primary-700">MediCare - Nurse</span>
          </div>
        ...
        <div className="grid md:grid-cols-4 gap-6">
          <aside className="md:col-span-1">
            <nav className="bg-white rounded-lg shadow-sm p-4 space-y-1">
              <NavLink href="/nurse" icon={<Activity />} label="Dashboard" />
              <NavLink href="/nurse/patients" icon={<Users />} label="Patients" />
              <NavLink href="/nurse/vitals" icon={<Activity />} label="Vitals" />
